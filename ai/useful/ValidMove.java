...
public class ValidMove {
    ...
    /**
     * Erstellt ein 64er-Array. Der Index (0-63) entspricht der Feldnummer
     * (1-64). Der Wert ist entweder die Farbe (0, 1) oder ein leeres Feld (-1)
     * 
     * @param situation
     * @return
     */
    public static byte[] getUsedFields(short[] situation) {
        /*
         * Alle 64 Felder des Schachfelds, mit folgender Information: -1=leer,
         * byte-Wert=entsprechende Farbe
         */
    ...
    public static LinkedList<short[]> pawnCheck(short[] situation) {
        ...
        for (int j = 0; j < situation.length; j++) {
            ...
            if (ExtractInformationFromBinary.getFigure(figure) == ChessfigureConstants.PAWN) {
                ...
                for (int i = 0; i < 4; i++) {
                    // TODO Oben rechts/links nur wenn Gegner auf entsprechendem
                    // Feld

                    // Neue Situation ---> short[] newSituation = new short[32]
                    // <--- erstellen. 32 Felder angenommenes Maximum.
                    short[] newSituation = situation.clone();

                    // Bauer versetzen (ausgelesene Position verändern, neuen
                    // Short-Wert erstellen)
                    int pos;
                    switch (i) {
                    case 0: // Bauer geht nach oben links
                        ...
                        if (!((color == 0 && usedFields[pos] == 1) || (color == 1 && usedFields[pos] == 0))) {
                            continue;
                        }
                    ...
                    // versetzten Bauern in newSituation einbinden (alten Bauern
                    // ersetzen)
