...
/**
 * Repraesentiert das Spielfeld und enthaelt eine HashMap mit allen Figuren und
...
public class Field
{
     ...
     * alle noetigen Figuren und positioniert sie auf dem Spielfeld.
     */
    private Field() {
    ...
    private void equipField() {
        ...
        for (int j = 1; j <= 16; j++) {
            if (j <= 8) {
                ...
                this.putFigureAt(position, new FigurePawn(
                        ChessfigureConstants.WHITE));
    ...
    public boolean putFigureAt(Integer fieldNumber, Figure figure) {
        if (!this.isFigureOnField(fieldNumber) && fieldNumber > 0
                && fieldNumber <= 64) {
            this.figures.put(fieldNumber, figure);
            return true;
        } else {
            System.out.println("Es ist eine Figur auf dem Feld (" + fieldNumber
                    + ") schon vorhanden.\n");
        ...
        try {
            ...
            if (!this.isFigureOnField(fromFieldNumber))
                throw new FieldException(
                        "Es befindet sich keine Figur auf dem Feld angegebenen Feld ("
                                + getFieldName(fromFieldNumber) + ")!");

            // Ist das Zielfeld frei?
            if (this.isFigureOnField(toFieldNumber))
                throw new FieldException(
                        "+++++\nEs befindet sich eine Figur auf Feld "
                                + getFieldName(toFieldNumber)
    ...
    public String getCurrentField() {
        ...
        for (int i = 0; i < sortedList.size(); i++) {
            ...
            str += "Feld " + getFieldName(fieldNumber) + "(" + fieldNumber
                    + "): " + f.toString() + "\n";
        }
    ...
    public short[] exportToShortArray() {
        ...
        Iterator<Entry<Integer, Figure>> it = this.figures.entrySet()
                .iterator();
        ...
        while (it.hasNext()) {
            ...
            short s = ChessfigureConstants.makeFigureShort(color, figureType,
                    positionX, positionY);
    ...
    public static byte getXPositionFromFieldnumber(int fieldNumber) {
        ...
        try {
            if (fieldNumber < 1 || fieldNumber > 64)
                throw new FieldException(
                        "Gegebene Feldwert ausserhalb des Feldes (1-64)!");
            else {
                // y-Wert ermitteln
        ...
        try {
            if (!Pattern.matches(pattern, fieldName))
                throw new FieldException(
                        "Die Eingabe ("
                                + fieldName
                                + ") entspricht nicht einer gueltigen Feldbezeichnung!\n Buchstaben klein geschrieben?");
            else {
                // Gueltige Feldbezeichnung, Zeile und Spalte "ausschneiden"
                int column = getColumnnumberByChar(fieldName.substring(0, 1)
                        .charAt(0));
    ...
    public static boolean isValidXPosition(char xPosition) {
        return (xPosition == 'a' || xPosition == 'b' || xPosition == 'c'
                || xPosition == 'd' || xPosition == 'e' || xPosition == 'f'
                || xPosition == 'g' || xPosition == 'h');
    }
